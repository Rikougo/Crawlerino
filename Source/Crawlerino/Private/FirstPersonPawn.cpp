...
void AFirstPersonPawn::BeginPlay()
{
	...
	SetActorLocation(FVector{(double)_TerrainPos.X * PawnMovementDelta, (double)_TerrainPos.Y * PawnMovementDelta, GetActorLocation().Z});
...
void AFirstPersonPawn::Tick(float DeltaTime)
{
	...
	if (_IsWalking)
	{
		...
		float AnimProgress = AnimTime / AnimationTime;
...
void AFirstPersonPawn::MoveCharacter(const FInputActionInstance& Instance)
{
	...
	if (_Dungeon->GetDungeonGrid().CanMoveTo(TargetPos.X, TargetPos.Y))
	{
		...
		{
			_TerrainPos.X += Sign;
			Pos.X += Sign * PawnMovementDelta;
		} else
		{
			_TerrainPos.Y += Sign;
			Pos.Y += Sign * PawnMovementDelta;
		}
